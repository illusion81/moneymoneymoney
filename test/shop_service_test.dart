...
void main() {
  ...
  group('ShopService', () {
    test('default items are owned and equipped in initialState', () {
      ...
      expect(state.equippedItemIds[ShopItemCategory.treeSkin], 'tree-classic-oak');
    ...
    test('purchase succeeds and deducts coins when balance and level both suffice', () {
      final state = service.initialState();
    ...
    test('purchase fails with insufficientCoins and leaves the balance untouched', () {
      final state = service.initialState();
      ...
      expect(result.state.ownedItemIds, isNot(contains('tree-golden-ginkgo')));
    });
    ...
    test('re-purchasing an owned item fails with alreadyOwned and does not deduct', () {
      final state = service.initialState();
    ...
    test('equipping replaces the previous item in the same category only', () {
      ...
      final equipped = service.equip(itemId: 'tree-golden-ginkgo', state: state);

      expect(equipped.equippedItemIds[ShopItemCategory.treeSkin], 'tree-golden-ginkgo');
      expect(equipped.equippedItemIds[ShopItemCategory.ground], 'ground-meadow');
