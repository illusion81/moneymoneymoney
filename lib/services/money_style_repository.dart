...
class SharedPreferencesMoneyStyleRepository implements MoneyStyleStore {
  ...
  Future<void> save(MoneyStyleCompletion c) async {
    final encoded = jsonEncode({
      'schemaVersion': 2,
      'questionVersion': 'money-style-v1',
      'userId': c.session.userId,
      'sessionId': c.session.sessionId,
      'selectedAnswerIds': c.session.answerIdsFor(moneyStyleQuestions),
  ...
  Future<MoneyStyleCompletion?> load() async {
    try {
      ...
      if (map['schemaVersion'] != 2 ||
          map['questionVersion'] != 'money-style-v1') {
        return null;
      }
      final answers = <int, int>{};
      for (final entry in (map['selectedAnswerIds'] as Map).entries) {
        final q = moneyStyleQuestions.firstWhere(
          (q) => q.id == int.parse('${entry.key}'),
        );
        final index = q.answers.indexWhere((a) => a.id == entry.value);
      ...
      return MoneyStyleCompletion(
        session: session,
        result: session.isCompleteFor(moneyStyleQuestions)
            ? MoneyStyleEngine().generateResult(session, moneyStyleQuestions)
            : null,
      );
    } catch (_) {
      return null;
